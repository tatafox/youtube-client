...
export class SortPipe implements PipeTransform {
  private sortByViews(cards: ISearchItem[], isIncrease: boolean): ISearchItem[] {
    if (!isIncrease) {
      return cards.sort((prev, next) => +next.statistics.viewCount - +prev.statistics.viewCount);
    }
    return cards.sort((prev, next) => +prev.statistics.viewCount - +next.statistics.viewCount);
  }

  private sortByDate(cards: ISearchItem[], isIncrease: boolean): ISearchItem[] {
    if (!isIncrease) {
      return cards.sort((prev, next) => (
  ...
  public transform(items: ISearchItem[], sortSettings: ISortSettings): ISearchItem[] {
    if (sortSettings.filterBy === sortMap.view) {
      return this.sortByViews(items, sortSettings.sortIncrease);
    } if (sortSettings.filterBy === sortMap.date) {
      return this.sortByDate(items, sortSettings.sortIncrease);
    }
    return items;
  }
}
